...
const MeatStorePage = (props: Props) => {
  const { data, refetch } = api.meatStore.getAll.useQuery();
  const [categoryInputValue, setCategoryInputValue] = useState("");
  const { mutateAsync: create } = api.meatStore.create.useMutation({
    onSuccess: async () => {
      await refetch();
    },
  });
  const { mutateAsync: deleteApi } = api.meatStore.delete.useMutation();
